...
# - Patched May 2026 to remove no-arg enableLayersReadout fallback and add branch logging
...
logger = logging.getLogger("v3.transport")
...
class V3Transport:
    ...
    async def _call_board_method(self, method_name: str, label: str, *args):
        method = getattr(self.board, method_name, None)
        if method is None:
            raise V3TransportError(f"boardDriver has no {method_name}()")

        try:
            awaitable = method(*args)
        except Exception as exc:  # noqa: BLE001
            self._raise_transport(f"{label} [argument binding]", exc)

        return await self._call(label, awaitable)
    ...
    async def enable_readout(self, *, autoread: bool = True, flush: bool = True) -> None:
        """
        Enable layer readout using explicit boardDriver signatures only.

        This intentionally does *not* fall back to parameterless
        enableLayersReadout(), because that can silently discard the caller's
        ownership intent (manual vs autoread) and make debugging very hard.
        """
        lanes = self.lanes
        attempts: list[tuple[str, tuple[Any, ...]]] = [
            (
                f"enableLayersReadout(autoread={autoread}, flush={flush})",
                (autoread, flush),
            ),
            (
                f"enableLayersReadout(lanes={lanes}, autoread={autoread}, flush={flush})",
                (lanes, autoread, flush),
            ),
        ]

        errors: list[str] = []
        for label, args in attempts:
            logger.info("Trying readout enable path: %s", label)
            try:
                await self._call_board_method("enableLayersReadout", label, *args)
                logger.info("Enabled readout using path: %s", label)
                return
            except V3TransportFatalError:
                logger.exception("Fatal transport error while trying readout enable path: %s", label)
                raise
            except V3TransportError as exc:
                logger.warning("Readout enable path failed: %s | %s", label, exc)
                errors.append(f"{label}: {exc}")

        raise V3TransportError(
            "Could not enable readout with explicit signatures only; "
            "parameterless enableLayersReadout() fallback is intentionally disabled. "
