...
This module is a Python port of the essential register-access and helper
functions from the original C test runner (`test_APIX.c`).  It is intentionally
kept separate from the AstroPix chip-control/readout stack: the adapter board is
used for probe-card contact, power-switch, current-monitor and HV-control
operations, not for normal AstroPix slow-control/readout.

Register protocol copied from the C runner
------------------------------------------
TCP port: 5000
Write:    [0x01, addr, data0, data1, data2, data3] with little-endian data
          then receive one acknowledgement byte.
...
- during contact test, only the TEST switch may be ON;
  AVSS/AVDD/DVDD must be OFF.
- after contact test, chip-test stages may enable AVSS/AVDD/DVDD; the TEST
  switch state is not part of the chip-power pass/fail criterion.
"""
...
class ProbeAdapterClient:
    ...
    def set_bias_voltage(
        ...
        *,
        ...
        Returns the DAC code written to REG_HV_DAC.  By default this uses a
        conservative placeholder linear 0..-500 V mapping; replace the
        calibration object after the adapter HV calibration is measured.
        """
